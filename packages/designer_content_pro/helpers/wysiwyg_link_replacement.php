...
//Functions copied from built-in "content" block controller (5.6.0.2)...
// (the only change made is replacing 'ContentBlockController' with __CLASS__)

class WysiwygLinkReplacementHelper {
	...
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array(__CLASS__, 'replaceFileIDInEditMode'),				
			$text);

		
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array(__CLASS__, 'replaceDownloadFileIDInEditMode'),				
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array(__CLASS__, 'replaceCollectionID'),				
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array(__CLASS__, 'replaceImageID'),				
			$text);

		// now we add in support for the files that we view inline			
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array(__CLASS__, 'replaceFileID'),				
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array(__CLASS__, 'replaceDownloadFileID'),				
			$text);
